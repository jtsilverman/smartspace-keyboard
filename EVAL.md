...
- `.` `?` `!` `,` `"` -- the four texting workhorses plus the opening quote. Comma is "the sentence continues here"; the opening quote is "quoted speech starts here" ("u say" + double-space) -- both are continuation predictions, structurally different from sentence enders (Jake, 2026-07-26).
- Deferred (`: ; -`): rare in texting relative to the rest, and each extra cycle candidate makes recovery slower. Revisit if opt-in export data shows real usage (Jake, 2026-07-26).
...
2. Sentence type: statement, wh-question, yes-no question, tag question, imperative, exclamation/greeting, mid-sentence continuation (comma), quote introducer (opening quote).
...
- **Real set** (`RealCorpus.swift`): sentences from the UCI SMS Spam Collection (ham only) whose senders typed the mark themselves; the mark is the label. Comma rows are text-before-a-comma inside real messages; quote rows are clause-anchored introducer phrases real senders followed with a quote (only 38 exist in the source -- thin but real; authored coverage grows with the rule work). Stratified so ? ! , have signal; natural texting is ~2/3 periods, so real-world scores skew higher than benchmark scores.
