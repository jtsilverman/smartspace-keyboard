...
final class KeyPopView: UIView {
    ...
    ///   coordinates; the pop frames itself over it.
    init(title: String, capFrame: CGRect) {
        let width = CalloutGeometry.bubbleWidth(keyWidth: capFrame.width)
        let height = CalloutGeometry.bubbleHeight + CalloutGeometry.neckHeight + capFrame.height
        super.init(frame: CGRect(x: capFrame.minX - CalloutGeometry.overhangPerSide,
                                 y: capFrame.maxY - height,
                                 width: width, height: height))
        isUserInteractionEnabled = false

        shape.path = Self.balloonPath(width: width, capHeight: capFrame.height).cgPath
    ...
    /// both sides, cap slice with the stock 5pt bottom corners.
    private static func balloonPath(width: CGFloat, capHeight: CGFloat) -> UIBezierPath {
        ...
        let capLeft = CalloutGeometry.overhangPerSide
        let capRight = width - CalloutGeometry.overhangPerSide
